	...
	for _, campaign := range campaigns {
		...
		for _, d := range decisions {
			...
			if campaign.AutoApproval {
				threshold = 0.65 // lower threshold in auto-approval campaigns
			}
			...
			// Risk gate: never auto-approve DANGER
			...
			// Execute the action via the Decision Action callback
			am.executeDecision(ctx, campaign, d)

			// Only approve one decision per cycle to avoid flooding
			break
		}
	}
}
...
func (am *AutoMode) isRiskAcceptable(d *types.Decision) bool {
	...
	// LOW: auto-approve in AutoApproval mode
	// MEDIUM: requires explicit auto-approval config
	// HIGH/DANGER: never auto-approve

	// Detect risk from technique name
	...
	case "safe", "low":
		return true
	case "medium":
		return am.cfg.AI.AutoApproval // only if user explicitly enabled
	default:
		return false
	}
}
